...
status_t CameraFlashlight::createFlashlightControl(const String8& cameraId) {
    ...
    if (mFlashControl == NULL) {
        ALOGE("%s: cannot create flash control for module api v2.3+",
                __FUNCTION__);
        return NO_MEMORY;
    }
...
status_t ModuleFlashControl::hasFlashUnit(const String8& cameraId, bool *hasFlash) {
    ...
    *hasFlash = false;
    ...
    /* Back camera */
    if (atoi(cameraId.string()) == 0) {
        ALOGD("%s: back camera has flash unit via sysfs", __FUNCTION__);
        *hasFlash = true;
    }
...
status_t ModuleFlashControl::setTorchMode(const String8& cameraId, bool enabled) {
    ALOGE("%s: set camera %s torch mode to %d", __FUNCTION__,
            cameraId.string(), enabled);
